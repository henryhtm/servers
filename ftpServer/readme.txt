...
Before running this container, you have to get the following ready:
   * A data directory with a sub directory(such as : /ftproot_dir/ftpdata_dir/) for upload/download files .
   * the root diretory should not be writable, use "chmod 555 ftproot_dir" to set the root directory  readable and excutable .
   * the data diretory should be writable, use "chmod 666 ftpdata_dir" to set the data directory  readable and writable .
   * Configure firewall of your host to open TCP port 21 .

The simplest where to use this image is using the following command:
   docker run -d -p 20-21:20-21 -v /ftproot_dir/:/mnt/ftp-dir CONTAINER_NAME

If you want to configure users and passwd, please specify the following USERLIST parameter:
   USERLIST=TYPE1,USER1,PASSWD1 TYPE2,USER2,PASSWD2

If you want to run ftp server in PASSIVE mode , please specify the following parameters and open the coordinary ports:
   FTPMODE=PASV 
...
Examples of command:
   * Run the ftp server with the default config(PORT mode, AdminUser:ftp-admin Passwd:a1313n ReadOnlyUser:ftp-read Passwd:rd1354)
      docker run -d -p 20-21:20-21 -v /ftproot_dir/:/mnt/ftp-dir CONTAINER_NAME
...
You can get the FTP users' info by command:
